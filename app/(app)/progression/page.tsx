...
import { listHistory, listRecords, muscleDistribution, progressionSeries, trainedExercises } from "@/lib/queries";
...
export default async function ProgressionPage({
  ...
  searchParams: Promise<{ exo?: string; periode?: string }>;
}) {
  ...
  const [series, records, distribution, history] = await Promise.all([
  ...
  const first = series[0]?.max ?? 0;
  const last = series.at(-1)?.max ?? 0;
  ...
  return (
    <div className="scroll">
      <div style={{ padding: "8px 20px 14px" }}>
        ...
        <h1 style={{ margin: "5px 0 0", font: "700 26px var(--sans)", letterSpacing: "-.8px" }}>Charge max</h1>
      </div>
      ...
      {current ? (
        <>
          ...
          <FilterChips
            ...
            param="periode"
            base="/progression"
            params={{ exo: current.slug, periode: periodLabel }}
            style="solid"
          />
          ...
          <section style={{ margin: "0 20px 16px", padding: "16px 14px 12px", borderRadius: 22, background: "var(--surf)", border: "1px solid var(--hair)" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 9, padding: "0 4px 10px", flexWrap: "wrap" }}>
              <span style={{ font: "700 32px var(--sans)", letterSpacing: "-1.2px" }}>
                {dec(last)}<span style={{ fontSize: 15, color: "var(--mut)" }}> {user.unit}</span>
              </span>
              {series.length > 1 && (
            ...
            <ProgressChart points={series.map((p) => ({ date: p.date, max: p.max }))} />
          </section>
